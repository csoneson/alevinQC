...
abstract: >
    `r Biocpkg("alevinQC")` reads output files from Alevin and generates summary reports.
vignette: >
...
application, containing the same information as the reports.

```{r v7, message=FALSE}
app <- alevinQCShiny(baseDir = baseDir, sampleId = "testSample")
```

Once created, the app can be launched using the `runApp()` function from the `r
CRANpkg("shiny")` package.
