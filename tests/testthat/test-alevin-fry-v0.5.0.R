test_that("checking for input files works", {
    ...
    tmp <- tempdir()
    file.copy(from = system.file("extdata/alevinfry_example_v0.5.0",
                                 package = "alevinQC"),
              to = tmp, overwrite = TRUE, recursive = TRUE)
    file.remove(file.path(tmp, "alevinfry_example_v0.5.0/map/cmd_info.json"))
    expect_error(checkAlevinFryInputFiles(
        mapDir = file.path(tmp, "alevinfry_example_v0.5.0", "map"),
        permitDir = file.path(tmp, "alevinfry_example_v0.5.0", "permit"),
        quantDir = file.path(tmp, "alevinfry_example_v0.5.0", "quant")))
})
